...
#         if zeroDict == {}:
#             return False
#         jumRecursion(arr,zeroDict,jumpedIndexes,lastIndex,start)
#         if False in zeroDict.values():
#             return False
#         else:
#             return True
# def jumRecursion (arr: list, zeroDict: dict, jumpedIndexes: list, lastIndex: int, curIndex: int):
#     if curIndex not in jumpedIndexes:
#         jumpedIndexes.append(curIndex)
#         if curIndex in zeroDict.keys():
#             zeroDict =zeroDict.clear()
#         if curIndex+arr[curIndex]<=lastIndex:
#             curIndex = curIndex+arr[curIndex]
#         # else:
#         #     curIndex = lastIndex - curIndex
#             jumRecursion(arr, zeroDict, jumpedIndexes, lastIndex, curIndex)
#         if curIndex - arr[curIndex]>=0:
#             curIndex = curIndex -arr[curIndex]
#         else:
#             curIndex = lastIndex-(arr[curIndex]-curIndex)
#             jumRecursion(arr, zeroDict, jumpedIndexes, lastIndex, curIndex)
    ...
    def canReach(self, arr: list, start: int) -> bool:
        ...
        while len(que) >= 1:
            curIndex = que.pop()
            if curIndex not in sett:
                sett.add(curIndex)
                if arr[curIndex] == 0:
                    return True
                if arr[curIndex]+curIndex<len(arr):
                    que.append(arr[curIndex]+curIndex)
                if curIndex - arr[curIndex]>=0:
                    que.append(curIndex-arr[curIndex])
        return False
